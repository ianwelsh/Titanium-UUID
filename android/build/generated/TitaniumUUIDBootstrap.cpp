...
#define TAG "it.scsoft.tiuuid"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> TitaniumUUID_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("TitaniumUUID.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TitaniumUUIDBindings::lookupGeneratedInit(
...
static void TitaniumUUID_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(TitaniumUUID_getBinding)->GetFunction());
}

static void TitaniumUUID_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TitaniumUUIDBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TitaniumUUIDBinding = {
	"it.scsoft.tiuuid",
	TitaniumUUID_init,
	TitaniumUUID_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_it_scsoft_tiuuid_TitaniumUUIDBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("it.scsoft.tiuuid", &TitaniumUUIDBinding);
	titanium::KrollBindings::addExternalLookup(&(::TitaniumUUIDBindings::lookupGeneratedInit));
}
